...
│   ├── analyze_comments.Rmd          # final comments analysis (all companies)
│   ├── analyze_news.Rmd              # final news analysis (all companies)
...
```
 ...
 comments_<t>_all_sentiments.csv  news_<t>_sentiments_S1/S2.csv
      ...
      analyze_comments.Rmd              analyze_news.Rmd
                ▼                                ▼
         comment tables                    news tables
```

FNSPID is the only true external input; the WSB comment dumps are produced by
`extract_reddit.py` (in the repo) and are just too large to commit. Each branch
ends in a single output: its result tables. The `analyze_*.Rmd` also pull live
market data (OHLC + volume) via `quantmod::getSymbols` (Yahoo) and handle the
Madrid→NY time-zone conversion internally.
